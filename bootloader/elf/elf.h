 ...
 * =====================================================================
 *
 * Перемещение — это процесс связывания символических
 * ссылок с символическими определениями. Например, когда программа
 * вызывает функцию, соответствующая инструкция вызова должна
 * передать управление на правильный адрес назначения во время
 * выполнения. Перемещаемые файлы должны содержать записи о
 * перемещении, которые описывают, как изменять содержимое секций,
 * что позволяет исполняемым и разделяемым объектным файлам
 * содержать корректную информацию для образа программы в памяти.
 *
 * Исполняемые файлы также могут содержать записи о перемещении,
 * которые необходимы, когда в коде есть несвязанные ссылки на
 * разделяемый объект или когда код является позиционно-независимым.
 *
 * Большинство перемещений являются символическими и заключаются
 * в вычислении значения выражения, включающего символ и смещение
 * (называемое аддендом), и применении результата к определённому
 * месту в объектном коде. Тип перемещения кодирует операцию (как
 * вычисляется выражение) и формат (как результат применяется к
 * местоположению).
 *
 * В исполняемых файлах некоторые перемещения являются относительными.
 * Относительное перемещение помечает местоположение, содержащее
 * 32-битный или 64-битный адрес, который должен быть перемещен,
 * когда сегмент программы загружается по адресу времени выполнения,
 * отличному от его адреса времени компоновки. Эти перемещения не
 * требуют символа или адденда.
 */
